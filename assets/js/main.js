  ...
  if (navMenu) {
    const setActiveNavLink = () => {
      ...
      navMenu.querySelectorAll('a').forEach(link => {
        ...
        if (currentPage === 'index.html' && (href === './' || href === '#')) {
          link.classList.add('is-active');
          link.setAttribute('aria-current', 'page');
        }
      });
    ...
    if (navActions && !navMenu.querySelector('.nav-menu-actions')) {
      const menuActions = document.createElement('div');
      menuActions.className = 'nav-menu-actions';
      navActions.querySelectorAll('a').forEach(actionLink => {
        menuActions.appendChild(actionLink.cloneNode(true));
      });
      navMenu.appendChild(menuActions);
    }
  }
